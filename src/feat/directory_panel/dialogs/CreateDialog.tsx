...
export function CreateDialog({ show, entity, onOpenChange, newItemType }: DialogProps & { newItemType: "dir" | "file" }) {
  ...
  async function confirm() {
    ...
    if (!validateDirectoryName(inputName)) {
      toast.error(t("create_dialog.invalid_name.title"), { description: t("create_dialog.invalid_name.description") });
      onOpenChange(false);
      setInputName("");
      return;
    }
    ...
    if (result) {
      ...
      toast.error(t("create_dialog.fail.title", { target_name: finalName }), { description: t("create_dialog.fail.description") });
    }
    onOpenChange(false);
    setInputName("");
  }

  const placeholder = newItemType === "dir" ? t("create_dialog.placeholder_folder") : t("create_dialog.placeholder_file");

  const title = newItemType === "dir" ? t("create_dialog.title_folder") : t("create_dialog.title_file");

  const description =
    newItemType === "dir" ? t("create_dialog.desc_folder", { dir_name: targetDir.name }) : t("create_dialog.desc_file", { dir_name: targetDir.name });
