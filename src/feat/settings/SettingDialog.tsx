...
const AboutApp = () => {
  ...
  return (
    <DialogDescription className="flex gap-2 items-center opacity-75 text-xs">
      <div>
        © Made with ❤️ by{" "}
        <Tooltip delayDuration={0} content={"https://github.com/greyovo"}>
      ...
      <div>
        Source:{" "}
        <Tooltip delayDuration={0} content={"https://github.com/greyovo/MarkditorApp"}>
      ...
      <DialogContent className="pr-3">
        ...
        <ScrollArea className="h-[55vh] my-5 pr-5">
          <Flex direction={"column"} gap={"4"} className=" overflow-y-auto">
            ...
            {/* Theme mode */}
            <SettingItem
              title={t("settings.theme_mode.title")}
              subtitle={t("settings.theme_mode.description")}
              trailing={
                <Select.Root value={prefThemeMode} onValueChange={(v) => prefActions.setThemeMode(v as PrefThemeMode)}>
            ...
            {/* Auto save */}
            <SettingItem
              title={t("settings.auto_save.title")}
              subtitle={t("settings.auto_save.description")}
              trailing={<Switch checked={autoSave} onCheckedChange={(v) => prefActions.toggleAutoSave(v)} />}
            />

            {/* Auto save interval */}
            <SettingItem
              className={!autoSave ? "opacity-45" : ""}
            ...
            <SettingItem
              title={t("settings.default_show_toolbar.title")}
              subtitle={t("settings.default_show_toolbar.description")}
              trailing={<Switch checked={defaultShowToolbar} onCheckedChange={(v) => prefActions.toggleDefaultShowToolbar(v)} />}
            />
