...
const HistoryItems = () => {
  ...
  function handleClearHistory() {
    toast(i18next.t("history.clear_confirm_text"), {
      ...
      position: "top-center",
      action: {
        label: <Button size={"1"}>{i18next.t("confirm")}</Button>,
        onClick: () => {
  ...
  return (
    <>
      ...
      {fileHistory.map((file) => (
        <DropdownMenu.Item onClick={() => handleOpenFile(file)} key={file}>
          {file}
        </DropdownMenu.Item>
      ))}
      ...
      {folderHistory.map((folder) => (
        <DropdownMenu.Item onClick={() => handleOpenFolder(folder)} key={folder}>
          {folder}
        </DropdownMenu.Item>
      ))}
      {!isAllEmpty && <DropdownMenu.Separator />}
      {!isAllEmpty && <DropdownMenu.Item onClick={handleClearHistory}>{t("titlebar_dropdown.clear_history")}</DropdownMenu.Item>}
    </>
  );
};
